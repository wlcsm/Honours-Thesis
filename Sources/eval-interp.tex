...
We will be interested in the particular case where $R$ is a polynomial ring and $I_j$ are linear, i.e. $I_j = (x - \alpha_j)$ for some $\alpha_j \in K$. In this case we can evaluate the map $R/(I_1\ldots I_k) \to R/(I_1) \times \cdots \times R/(I_k)$ by evaluating the polynomials at the points $\alpha_j$.
